...
void MaCh3_LLH(std::string &MaCh3File) {
  ...
  for (unsigned int i = 0; i < Params; ++i) {
    ...
    TH1D *FluxGraph = (TH1D*)(MaCh3->Get((std::string("Flux_LLH/")+MaCh3Names[i]+"_flux").c_str()));
    TH1D *XsecGraph = (TH1D*)(MaCh3->Get((std::string("XSec_LLH/")+MaCh3Names[i]+"_xs").c_str()));
    TH1D *ND280Graph = (TH1D*)(MaCh3->Get((std::string("ND280_LLH/")+MaCh3Names[i]+"_nd").c_str()));
    TH1D *SampleGraph = (TH1D*)(MaCh3->Get((std::string("Sample_LLH/")+MaCh3Names[i]+"_sam").c_str()));
    TH1D *TotalGraph = (TH1D*)(MaCh3->Get((std::string("Total_LLH/")+MaCh3Names[i]+"_full").c_str()));
...
std::vector<std::string> MakeBANFFNames() {
  ...
  NameVector.push_back("EB_dial_C");
  NameVector.push_back("EB_dial_O");
...
std::vector<double> GetXSecNominal() {
  ...
  std::string XsecCovName = "../inputs/xsec_covariance_2020a_v3.root";
