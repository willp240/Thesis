...
void throwBins(std::string file0){
  ...
  TFile *treefile = new TFile("fluctatedevents.root", "RECREATE");

  double p,t;
  int sam;
  TTree *t1 = new TTree("events", "Events");
  t1->Branch("momentum",&p);
  t1->Branch("costheta",&t);
  t1->Branch("sample",&sam);
  ...
  for(int s=0; s<nsamples; s++){
	...
	p = (double)((maxmom-minmom)*rnd->Rndm()) + minmom;
	t = (double)((maxang-minang)*rnd->Rndm()) + minang;
