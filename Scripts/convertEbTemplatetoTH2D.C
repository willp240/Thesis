...
void convertEbTemplatetoTH2D (std::string PolyFileName){
  ...
  std::string targ[2] = {"C","O"};
  std::string spec[4] = {"numu", "numubar", "nue", "nuebar"};
  ...
  for(int s = 0; s<nsamples; s++){//nsamples; s++){
    TDirectory *ebtempdir = fileTH2D->mkdir(("MC_"+sampleName[s]+"_EbTemplates").c_str());
    ebtempdir->cd();
    for(int j_targ = 0; j_targ<2; j_targ++) {      
      for(int k = 0; k<4; k++) {
	TDirectory *ebtempsubdir = ebtempdir->mkdir((""+spec[k]+"_"+targ[j_targ]).c_str());
        ebtempsubdir->cd();
	std::cout << "Doing for " << sampleName[s] << " " << spec[k] << " " << targ[j_targ] << std::endl;
	for(int l = 0; l<73; l++) {
	  ebtempsubdir->cd();
	  std::string binlabel = Form("_Bin_%d", l);
	  //get the poly
	  std::string getname = "MC_" + sampleName[s] + "_EbTemplates/" + spec[k] + "_" + targ[j_targ] + "/MC_" + sampleName[s] + "_CCQE_Target_" + targ[j_targ] + "_" + spec[k] + binlabel;
	  std::string histname = "MC_" + sampleName[s] + "_CCQE_Target_" + targ[j_targ] + "_" + spec[k] + binlabel;
	  //	  std::cout << "Doing for " << getname << std::endl;	  
	  ...
	  //make the th2d
	  TH2D* hist = new TH2D(getname.c_str(), getname.c_str(), xedges.size()-1, &xbins[0], yedges.size()-1, &ybins[0]);
    ...
    xunderflow+=m[1*/
	  ...
	  //	  std::cout << "Made the TH2D for " << histname << "!" << std::endl;
