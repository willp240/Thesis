...
void GetAsimovPlots_HPDOnly2Fits(std::string FileName1, std::string FileName2) {
  ...
  if (Two != NULL) leg->AddEntry(Two, "Beeston-Barlow", "lpf");
  if (Three != NULL) leg->AddEntry(Three, "Poisson Likelihood", "lpf");
...
Then for the postfits (Histo named Two) they haven't been shifted yet. So we do the scaling then shift the central value. 
  */
  ...
  //and hard code fixed 2p2hEdep
  /*  Three->SetBinContent(nFluxParams+7,-999);
  Three->SetBinError(nFluxParams+7,-999);
  Three->SetBinContent(nFluxParams+8,-999);
  Three->SetBinError(nFluxParams+8,-999);
  Three->SetBinContent(nFluxParams+9,-999);
  Three->SetBinError(nFluxParams+9,-999);
  Three->SetBinContent(nFluxParams+10,-999);
  Three->SetBinError(nFluxParams+10,-999);*/
