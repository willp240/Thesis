...
void GetAsimovPlots_HPDOnly4Fits(std::string FileName1, std::string FileName2, std::string FileName3, std::string FileName4) {
  ...
  TH1D *Five = (TH1D*)File4->Get("param_flux_HPD_0");
  ...
  Two->SetFillColor(kBlack);
  //  Two->SetFillStyle(3003);
  Two->SetMarkerColor(kBlack);
  Two->SetMarkerStyle(7);
  Two->SetLineColor(kBlack);
  Three->SetMarkerColor(kRed);
  Three->SetLineColor(kRed);
  Three->SetMarkerStyle(7);
  Four->SetMarkerColor(kBlue);
  Four->SetLineColor(kBlue);
  Four->SetFillColor(kBlue);
  Four->SetMarkerStyle(7);
  Five->SetMarkerColor(kGreen);
  Five->SetLineColor(kGreen);
  Five->SetFillColor(kGreen);
  Five->SetMarkerStyle(7);

  // Make a Legend page
  TLegend *leg = new TLegend(0.0, 0.0, 1.0, 1.0);
  if (One != NULL) leg->AddEntry(One, "Prior", "lpf");
  if (Two != NULL) leg->AddEntry(Two, "4238 Bins (Fit Binning)", "lp");
  if (Three != NULL) leg->AddEntry(Three, "1347 Bins", "lp");
  if (Four != NULL) leg->AddEntry(Four, "574 Bins", "lp");
  if (Five != NULL) leg->AddEntry(Five, "179 Bins", "lp");

  canv->cd();
  canv->Clear();
  leg->Draw();
  canv->Print("detcovbinfits_leg.pdf");
  delete leg;
  ...
  for (int i = 0; i < nFluxPlots; ++i) {
    ...
    if (Five != NULL) FiveCopy[i] = PrettifyMe(Five, i);
    ...
    if (Five != NULL) FiveCopy[i]->Draw("e1,same");
    ...
    Ratio3->Draw("p same");
    TH1D *Ratio4 = MakeRatioPlot(OneCopy[i], FiveCopy[i]);
    ...
    canv->Print(("detcovbinflux_"+fluxnum+".pdf").c_str());
...
Then for the postfits (Histo named Two) they haven't been shifted yet. So we do the scaling then shift the central value. 
  ...
  */
  ...
  for (int i = 1; i < XsecPlots; ++i) {
    ...
    if (Five != NULL) {
      Five->GetXaxis()->SetRangeUser(XsecOffset[i-1], XsecOffset[i]);
      Five->Draw("e1,same");
    }
    ...
    TH1D *RatioFour = MakeRatioPlot(One, Five);
    ...
    RatioFour->SetMarkerColor(RatioFour->GetLineColor());
    RatioFour->Draw("p same");
    ...
    canv->Print(("detcovbinxsec_"+xsecnum+".pdf").c_str());
