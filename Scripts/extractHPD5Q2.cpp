...
void extractHPD5Q2(std::string inputfile);
...
void extractHPD5Q2(std::string inputFile) {
  ...
  TVectorD* HPD_mean_vec  = new TVectorD(ndraw-2);
  TVectorD* HPD_err_vec   = new TVectorD(ndraw-2);
  TVectorD* xsec_nom  = new TVectorD(nxsec);

  // Only want to draw covariance of xsec parameters!
  TMatrixT<double>* covariance = new TMatrixT<double>(ndraw-2,ndraw-2);
  TMatrixT<double>* correlation = new TMatrixT<double>(ndraw-2,ndraw-2);
  ...
  for(int i = 0; i < ndraw; ++i) {
    ...
    if (bnames[i]==("xsec_16") || bnames[i]==("xsec_18")) {
      continue;
    }
    if (bnames[i]==("xsec_15") || bnames[i]==("xsec_17")) {
	postEbindex--;
    }
    ...
    if (bnames[i] == "xsec_0" || bnames[i] == "xsec_4" || bnames[i] == "xsec_5" || bnames[i] == "xsec_6" || bnames[i] == "xsec_7" || bnames[i] == "xsec_8" || bnames[i] == "xsec_9" || bnames[i] == "xsec_19" || bnames[i] == "xsec_20" || bnames[i] == "xsec_21" || bnames[i] == "xsec_22" || bnames[i] == "xsec_27" || bnames[i] == "xsec_28" || bnames[i] == "xsec_29" || bnames[i] == "xsec_39" || bnames[i] == "xsec_40" || bnames[i] == "xsec_41" || bnames[i] == "xsec_42" || bnames[i] == "xsec_43" ) {
      peakval = peakval -1.0;
    }
  ...
  TVectorD* FittedParameters0  = new TVectorD(ndraw-2);
  TMatrixT<double>* FittedCovariance0 = new TMatrixT<double>(ndraw-2,ndraw-2);
  ...
  for (int i=nflux; i<nflux+nddet; i++) {
    (*FittedParameters0)(i) = (*HPD_mean_vec)(nflux+nxsec-2+(*th2ddetindex)(i-nflux));
    (*FittedCovariance0)(i,i) = (*covariance)(nflux+nxsec-2+(*th2ddetindex)(i-nflux),nflux+nxsec-2+(*th2ddetindex)(i-nflux));
    std::cout << (*th2ddetindex)(i-nflux) << std::endl;
  }
  for (int i=nflux+nddet; i<nflux+nddet+5; i++) {
    int c = i-nflux-nddet;
    (*FittedParameters0)(i) = (*HPD_mean_vec)(nflux+nxsec-2-5+c);
    (*FittedCovariance0)(i,i) = (*covariance)(nflux+nxsec-2-5+c,nflux+nxsec-2-5+c);
    std::cout << nxsec-5+c << std::endl;
  }
  for (int i=nflux+nddet+5; i<nflux+nddet+15; i++) {
    int c = i-nflux-nddet;
    (*FittedParameters0)(i) = (*HPD_mean_vec)(nflux+c);
    (*FittedCovariance0)(i,i) = (*covariance)(nflux+c,nflux+c);
    std::cout << c << std::endl;
  }
  for (int i=nflux+nddet+15; i<nflux+nddet+nxsec-4; i++) {
    int c = i-nflux-nddet;
    (*FittedParameters0)(i) = (*HPD_mean_vec)(nflux+c+2);
    (*FittedCovariance0)(i,i) = (*covariance)(nflux+c+2,nflux+c+2);
    std::cout << c+2 << std::endl;
  }
  for (int i=nflux+nddet+nxsec-5; i<nflux+nddet+nxsec-3; i++) {
    int c = i-nflux-nddet-nxsec+5;
    (*FittedParameters0)(i) = (*HPD_mean_vec)(nflux+15+c);
    (*FittedCovariance0)(i,i) = (*covariance)(nflux+15+c,nflux+15+c);
    std::cout << 15+c << std::endl;
    }
