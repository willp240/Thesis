...
void GetAsimovPlots_FourFits(std::string FileName1, std::string Name1, std::string FileName2, std::string Name2, std::string FileName3, std::string Name3) {
  ...
  canv->Print((FileName1+FileName2+FileName3+".pdf[").c_str());
  ...
  if (Four != NULL)
    {
      Four->SetFillStyle(0);
      Four->SetFillColor(kBlack);
      Four->SetMarkerColor(kBlack);
      Four->SetMarkerStyle(25);
      Four->SetLineWidth(2);
      Four->SetMarkerSize((One->GetMarkerSize())*0.5);
      Four->SetLineColor(Four->GetFillColor());
    }

  // Make a Legend page
  TLegend *leg = new TLegend(0.0, 0.0, 1.0, 1.0);
  if (One != NULL) leg->AddEntry(One, "Prior", "lpf");
  if (Two != NULL) leg->AddEntry(Two, Name1.c_str(), "pf");
  if (Three != NULL) leg->AddEntry(Three, Name2.c_str(), "pf");
  if (Four != NULL) leg->AddEntry(Four, Name3.c_str(), "lep");
  canv->cd();
  canv->Clear();
  leg->Draw();
  canv->Print((FileName1+FileName2+FileName3+".pdf").c_str());
  ...
  int XsecOffset[XsecPlots] = {nFluxParams, nFluxParams+13, nFluxParams+13+8, nFluxParams+13+8+4, nBins};
