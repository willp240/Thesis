...
The prior predicted rates were also calculated, using the same method as for ND280, but throwing the SK flux, SK detector, and interaction uncertainties and reweighting the SK MC.

The total prior and posterior event rates for each of the SK samples are shown in Table \ref{}. The total number of events has been reduced/increased in X samples, beyond the prior uncertainties. The uncertainties for all samples have been reduced significantly, as shown in Table \ref{}, and the overall event rate uncertainty has been reduced from X$\%$ to $Y\%$ by the near detector fit. 
%table rates
%table errors

The prior and posterior distributions are shown in Figure \ref{}. The error band has been reduced for all samples, particularly in X regions of Y samples. %Impact on osc params
%plots
