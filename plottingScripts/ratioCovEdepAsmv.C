...
void ratioCovEdepAsmv() {
  ...
  TFile *m3file = TFile::Open("inputFiles/27AprTH2D574Asimov_drawCorr.root");
  TFile *bfile = TFile::Open("inputFiles/postfitfile_Asimov.root");  

  TObjArray* param_list = (TObjArray*)bfile->Get("param_list");

  for(int i=0; i<147; i++){
    std::cout << i << " " ;
    param_list->At(MaCh3ToBanff(i))->Print();
  }
  
  TMatrixT<double>* banffcov = (TMatrixT<double>*)bfile->Get("postfit_cov");
  TH2D* banffcov2 = (TH2D*)m3file->Get("postfit_cov_plot")->Clone();
  ...
  for(int i=1; i<=mach3cov->GetXaxis()->GetNbins(); i++){
    for(int j=1; j<=mach3cov->GetYaxis()->GetNbins(); j++){
      ...
      int banff_i = MaCh3ToBanff(i-1);
      int banff_j = MaCh3ToBanff(j-1);
      int mach3_i = i;
      int mach3_j = j;

      double m3 = mach3cov->GetBinContent(mach3_i,mach3_j)/(sqrt(mach3cov->GetBinContent(mach3_i,mach3_i)*mach3cov->GetBinContent(mach3_j,mach3_j)));
      double b = banffcov(banff_i, banff_j)/sqrt(banffcov(banff_i, banff_i)*banffcov(banff_j, banff_j));
      if((i>106&&i<111)||(j>106&&j<111)){
	if(i==j)
	  b=1;
	else
	  b=0;
      ...
      //      if(i==129||i==130){
      //std::cout << i << " " << mach3cov->GetBinContent(i,j) << " " << mach3cov->GetBinContent(i,i) << " " << mach3cov->GetBinContent(j,j) << " " << m3 << std::endl;
      ///std::cout << i << " " << banffcov(banff_i,banff_j) << " " << banffcov(banff_i,banff_i) << " " << banffcov(banff_j,banff_j) << " " << b << std::endl;
      //}

      mach3cov2->SetBinContent(mach3_i,mach3_j,m3);
      banffcov2->SetBinContent(mach3_i,mach3_j,b);
      ratio->SetBinContent(mach3_i,mach3_j,(m3-b));
      
      /*      if(i==125||i==142||j==125||j==142){
	mach3cov2->SetBinContent(i,j,0);
	banffcov2->SetBinContent(i,j,0);
	ratio->SetBinContent(i,j,0);

	if(i==j){
	  mach3cov2->SetBinContent(i,j,1);
	  banffcov2->SetBinContent(i,j,1);
	  ratio->SetBinContent(i,j,0);
	}
	}*/
    }
  ...
  ratio->Draw("colz");
  c1->Print("Mach3BanffAsmvCorrDifference.png");
  ratio->GetXaxis()->SetRangeUser(100,147);
  ratio->GetYaxis()->SetRangeUser(100,147);
  ratio->Draw("colz");
  c1->Print("Mach3BanffAsmvCorrDifferenceXsec.png");
  /*
  banffcov2->SetTitle("");
  banffcov2->GetZaxis()->SetTitle("Correlation");
  banffcov2->GetZaxis()->SetRangeUser(-1.0,1.0);
  PrettifyTitles(banffcov2);
  banffcov2->Draw("colz");
  c1->Print("BanffDataCorr.png");
  banffcov2->GetXaxis()->SetRangeUser(100,147);
  banffcov2->GetYaxis()->SetRangeUser(100,147);
  banffcov2->Draw("colz");
  c1->Print("BanffDataCorrXsec.png");
  c->cd();
  mach3cov2->SetTitle("");
...
int MaCh3ToBanff(int mach3index){
  ...
  else if(mach3index>=100 && mach3index<106)
    banffindex=mach3index+574+5;

  else if(mach3index>=106 && mach3index<118)
    banffindex=mach3index+574+5-4;

  else if(mach3index>=118 && mach3index<122)
    banffindex=mach3index+574+5+20-4;

  else if(mach3index>=122 &&mach3index<142)
    banffindex=mach3index+574+5-4-4;
