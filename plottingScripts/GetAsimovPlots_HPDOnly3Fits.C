...
void GetAsimovPlots_HPDOnly3Fits(std::string FileName1, std::string FileName2, std::string FileName3) {
  ...
  if (Two != NULL) leg->AddEntry(Two, "TH2D Fit Bin, 574 Det Bin", "lpf");
  if (Three != NULL) leg->AddEntry(Three, "TH2D Fit Bin, TH2D Fit Det Bin", "lpf");
  if (Four != NULL) leg->AddEntry(Four, "TH2Poly Fit Bin, TH2Poly Fit Det Bin", "lpf");
