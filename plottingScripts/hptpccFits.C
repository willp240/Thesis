...
void PrettifyTitles(TH1D *Hist) {
  ...
  for (int i = 0; i < Hist->GetXaxis()->GetNbins(); ++i) {
    ...
    else if (title == "FSI_INEL_LO_E") title = "FSI Inel lo";
    else if (title == "FSI_INEL_HI_E") title = "FSI Inel hi";
    else if (title == "FSI_PI_PROD") title = "FSI Pi Prod.";
    else if (title == "FSI_PI_ABS") title = "FSI Pi Abs.";
    else if (title == "FSI_CEX_LO_E") title = "FSI Cex lo";
    else if (title == "FSI_CEX_HI_E") title = "FSI Cex hi";
    ...
    else if (title == "pF_C") title = "#it{p}_{F} C";
    else if (title == "pF_O") title = "#it{p}_{F} O";
    else if (title == "MEC_C") title = "MEC C";
    else if (title == "MEC_O") title = "MEC O";    
    else if (title == "EB_C") title = "#it{E_{b}} C";
    else if (title == "EB_O") title = "#it{E_{b}} O";
    else if (title == "NUBAR_MEC_0") title = "MEC #bar{#nu}_{e}";
    ...
    else if (title == "CCCOH_C_0") title = "CC coh. ^{12}C";
    else if (title == "CCCOH_O_0") title = "CC coh. ^{16}O";
    else if (title == "NCCOH_0") title = "NC coh.";
    else if (title == "NC1GAMMA_0") title = "NC 1#gamma";
    else if (title == "NCOTHER_0") title = "NC oth.";
...
void GetAsimov3Fits2015(std::string FileName1, std::string FileName2, std::string FileName3) {
  ...
  if (Two != NULL) leg->AddEntry(Two, "ND280", "lpf");
  if (Three != NULL) leg->AddEntry(Three, "HPTPC #it{p}_{#mu}-cos#theta_{#mu}", "lpf");
  if (Four != NULL) leg->AddEntry(Four, "HPTPC STV", "lp");

  canv->cd();
  canv->Clear();
  leg->Draw();
  //canv->Print((FileName1+"_"+FileName2+"_"+FileName3+".pdf").c_str());
  canv->Print("hptpcfits_leg.pdf");
  delete leg;

  int nBins = One->GetXaxis()->GetNbins();

  /*  int nFluxParams=100;
  for(i=nFluxParams+17; i<=nBins; i++){
    Two->SetBinContent(i,1+(Two->GetBinContent(i)));
    Three->SetBinContent(i,1+(Three->GetBinContent(i)));
    Four->SetBinContent(i,1+(Four->GetBinContent(i)));
  }

  Two->SetBinContent(nFluxParams+12,1+(Two->GetBinContent(nFluxParams+12)));
  Three->SetBinContent(nFluxParams+12,1+(Three->GetBinContent(nFluxParams+12)));
  Four->SetBinContent(nFluxParams+12,1+(Four->GetBinContent(nFluxParams+12)));

  One->SetBinContent(nFluxParams+3,1.0);

  for(i=0; i<nBins; i++){
    Three->SetBinError(i,0.85*(Three->GetBinError(i)));
    Four->SetBinError(i,0.75*(Four->GetBinError(i)));
    if(i==105||i==106||i==107||i==114){
      Three->SetBinError(i,(Two->GetBinError(i)));
      Four->SetBinError(i,(Two->GetBinError(i)));
    }
    }*/
  ...
  for (int i = 0; i < nFluxPlots; ++i) {
    ...
    canv->Print(("hptpcfits_"+fluxnum+".pdf").c_str());
    //canv->Print((FileName1+"_"+FileName2+"_"+FileName3+".pdf").c_str());
...
Then for the postfits (Histo named Two) they haven't been shifted yet. So we do the scaling then shift the central value. 
  */
  ...
  PrettifyTitles(One);
  const int XsecPlots = 4;
  int XsecOffset[XsecPlots] = {nFluxParams, nFluxParams+7, nFluxParams+7+9, nBins};
  std::string titles[XsecPlots-1] = {"CC0#pi", "CC1#pi, #nu_{e}, CC DIS, CC coh, NC", "Pion FSI"};
  ...
  for (int i = 1; i < XsecPlots; ++i) {
    ...
    canv->Print(("hptpcfits_"+xsecnum+".pdf").c_str());
    //canv->Print((FileName1+"_"+FileName2+"_"+FileName3+".pdf").c_str());
    delete axis;
    delete line;
    delete line2;
