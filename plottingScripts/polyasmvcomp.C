...
void polyasmvcomp(std::string FileName1, std::string FileName2, std::string FileName3) {
  ...
  if (Two != NULL) leg->AddEntry(Two, "Uniform Fit Bins, 574 Det. Bins", "lpf");
  if (Three != NULL) leg->AddEntry(Three, "Non-Uniform Fit Bins, 574 Det. Bins", "lpf");
  if (Four != NULL) leg->AddEntry(Four, "Non-Uniform Fit and Det. Bins", "lp");

  canv->cd();
  canv->Clear();
  leg->Draw();
  canv->Print("polyasmvs_leg.pdf");
  delete leg;
  ...
  for (int i = 0; i < nFluxPlots; ++i) {
    ...
    canv->Print(("polyasmvsflux_"+fluxnum+".pdf").c_str());
...
Then for the postfits (Histo named Two) they haven't been shifted yet. So we do the scaling then shift the central value. 
  */
  ...
  for (int i = 1; i < XsecPlots; ++i) {
    ...
    canv->Print(("polyasmvsxsec_"+xsecnum+".pdf").c_str());
