...
void rhcmpdata28(std::string FileName1) {
  ...
  TH1D *Two = (TH1D*)File1->Get("postfit_params_plot");
  TH1D *Three = (TH1D*)File1->Get("postfit_params_plot_2");
  ...
  for (int i = 0; i < nFluxPlots; ++i) {
    ...
    //canv->Print((FileName1+FileName2+".pdf").c_str());
    canv->Print(("rhcmpdat28flux_"+fluxnum+".pdf").c_str());
  ...
  for (int i = 1; i < XsecPlots; ++i) {
    ...
    //canv->Print((FileName1+FileName2+".pdf").c_str());
    canv->Print(("rhcmpdatxsec28_"+xsecnum+".pdf").c_str());
