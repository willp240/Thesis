...
void checkpostpred(std::string file0) {

  //  gStyle->SetPalette(51);

  TCanvas *c = new TCanvas("canv", "canv", 1580, 1080);
  TFile *file_ = TFile::Open(file0.c_str());
  TFile *outfile = new TFile("outtest.root","RECREATE");
  ...
  for(int s=0; s<1; s++){
    ...
    TH2Poly* poly = (TH2Poly*)file_->Get(name.c_str())->Clone();

    double rms_sum=0;
    
    for (int b=1; b<29; b++){//poly->GetNumberOfBins(); b++){
	...
	TH2Poly* poly2 = (TH2Poly*)file_->Get(name3.c_str())->Clone();	
	events->Fill(poly2->GetBinContent(b));
	delete poly2;
      }
