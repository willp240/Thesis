...
void fhcrhcfits(std::string FileName1, std::string FileName2, std::string FileName3) {
  ...
  if (Two != NULL) leg->AddEntry(Two, "FHC + RHC", "lpf");
  if (Three != NULL) leg->AddEntry(Three, "FHC", "lpf");
  if (Four != NULL) leg->AddEntry(Four, "RHC", "lp");

  canv->cd();
  canv->Clear();
  leg->Draw();
  canv->Print("fhcrhcfits_leg.pdf");
  delete leg;
  ...
  for (int i = 0; i < nFluxPlots; ++i) {
    ...
    canv->Print(("fhcrhcfitsflux_"+fluxnum+".pdf").c_str());
...
Then for the postfits (Histo named Two) they haven't been shifted yet. So we do the scaling then shift the central value. 
  */
  ...
  for (int i = 1; i < XsecPlots; ++i) {
    ...
    canv->Print(("fhcrhcfitsxsec_"+xsecnum+".pdf").c_str());
