...
void pvalsample(std::string file0) {
  ...
  std::string samp_us[18] = {"FGD1_numuCC_0pi", "FGD1_numuCC_1pi", "FGD1_numuCC_other", "FGD1_anti_numuCC_0pi", "FGD1_anti_numuCC_1pi", "FGD1_anti_numuCC_other", "FGD1_NuMuBkg_CC0pi_in_AntiNu_Mode", "FGD1_NuMuBkg_CC1pi_in_AntiNu_Mode", "FGD1_NuMuBkg_CCother_in_AntiNu_Mode", "FGD2_numuCC_0pi", "FGD2_numuCC_1pi", "FGD2_numuCC_other", "FGD2_anti_numuCC_0pi","FGD2_anti_numuCC_1pi", "FGD2_anti_numuCC_other", "FGD2_NuMuBkg_CC0pi_in_AntiNu_Mode", "FGD2_NuMuBkg_CC1pi_in_AntiNu_Mode", "FGD2_NuMuBkg_CCother_in_AntiNu_Mode"};

  for(int s=0; s<18; s++){

    std::string name = samp[s] + "/" + samp_us[s] + "_drawfluc_draw";
    //    name = "FGD1 anti-numuCC 1pi/FGD1_anti_numuCC_1pi_drawfluc_draw";

    std::cout << name << std::endl;

    TH2D* hist = (TH2D*)file_->Get(name.c_str());
    ...
    hist->Draw("colz");
    c->Print((std::string("pvalsample_")+samp_us[s]+std::string(".pdf")).c_str());
