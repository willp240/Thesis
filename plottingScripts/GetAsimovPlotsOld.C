...
void GetAsimovPlots(std::string FileName, std::string FileName2) {
  ...
  TH1D *One = (TH1D*)File->Get("param_flux_prefit_0");
  TH1D *Two = (TH1D*)File->Get("param_flux_0");
  TH1D *Three = (TH1D*)File->Get("param_flux_gaus_0");
  TH1D *Four = (TH1D*)File->Get("param_flux_HPD_0");
  if (Four != NULL) Four->SetLineColor(kBlack);

  // Make a Legend page
  TLegend *leg = new TLegend(0.0, 0.0, 1.0, 1.0);
  if (One != NULL) leg->AddEntry(One, "Prior", "lpf");
  if (Two != NULL) leg->AddEntry(Two, "Arithmetic", "pf");
  if (Three != NULL) leg->AddEntry(Three, "Gauss", "pf");
  if (Four != NULL) leg->AddEntry(Four, "HPD", "lep");
  canv->cd();
  canv->Clear();
  leg->Draw();
  canv->Print((FileName+".pdf").c_str());
