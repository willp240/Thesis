...
void ratioCov() {
  ...
  TFile *bfile = TFile::Open("/vols/t2k/users/wparker2/irods-legacy/iRODS/BANFF_postfit_OA2020_AllParams_20200409.root");  
  ...
  for(int i=1; i<=mach3cov->GetXaxis()->GetNbins(); i++){
    for(int j=1; j<=mach3cov->GetYaxis()->GetNbins(); j++){
      ...
      if(i==107||i==108||i==109||i==110||i==125||i==142||j==107||j==108||j==109||j==110||j==125||j==142){
