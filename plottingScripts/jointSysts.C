...
void GetAsimovPlots_HPDOnly2Fits(std::string FileName1, std::string FileName2) {
  ...
  if (Two != NULL) leg->AddEntry(Two, "ND Only", "lpf");
  if (Three != NULL) leg->AddEntry(Three, "Joint Fit", "lpf");

  canv->cd();
  canv->Clear();
  leg->Draw();
  canv->Print("joint_leg.pdf");
  //  canv->Print((FileName1+"_"+FileName2+".pdf").c_str());
  delete leg;
  ...
  for (int i = 0; i < nFluxPlots; ++i) {
    ...
    canv->Print(("jointflux"+fluxnum+".pdf").c_str());
...
Then for the postfits (Histo named Two) they haven't been shifted yet. So we do the scaling then shift the central value. 
  */
  ...
  for (int i = 1; i < XsecPlots; ++i) {
    ...
    canv->Print(("jointxsec"+xsecnum+".pdf").c_str());
